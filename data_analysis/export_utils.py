...
def create_demographics_table(data):
    ...
    if 'education' in data.columns:
        try:
            education_counts = data.groupby(['group', 'education']).size().unstack(fill_value=0)
        except ValueError:
    ...
    if 'ethnicity' in data.columns:
        try:
            ethnicity_counts = data.groupby(['group', 'ethnicity']).size().unstack(fill_value=0)
        except ValueError:
            # Handle case where there might be issues with grouping
...
def export_tables_to_latex(participant_data, output_dir='.'):
    ...
    try:
        ...
        # Demographics table
        demo_df, edu_counts, eth_counts = create_demographics_table(participant_data)
...
"""
        ...
        if stress_results:
            max_effect_size = max(max_effect_size, abs(stress_results['effect_size']))
...
def create_methods_section(participant_data):
    ...
    control_count = len(participant_data[participant_data['group'] == 'Control'])
...
def export_publication_materials(participant_data, output_dir='.'):
    """
...
"""
        ...
        # Add stress results if available
        if 'post_stress' in participant_data.columns:
            stress_results = run_statistical_tests(participant_data, measure_col='post_stress')
